...
class ImageField {
  ...
  whenRead() {
    ...
    for (let img of collect.list) {
      if (img.loadState == LOAD_STATE_ERROR) {
        continue;
      } else if (img.loadState == LOAD_STATE_NONE) {
        return;
      } else if (img.loadState == LOAD_STATE_OPENED) {
        return;
  ...
  fillData() {
    if (this.palette == null) {
      throw new Error("imageField cannot have data filled without palette");
    }
...
module.exports.LOAD_STATE_NONE = LOAD_STATE_NONE;
module.exports.LOAD_STATE_OPENED = LOAD_STATE_OPENED;
